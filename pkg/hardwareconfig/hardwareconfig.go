...
func (hcm *HardwareConfigManager) ApplyConditionForProfile(nodeProfile *ptpv1.PtpProfile, conditionType string, sourceName string) error {
	if nodeProfile.Name == nil {
		return fmt.Errorf("PTP profile has no name")
	}
	...
	glog.Infof("Applying condition '%s' for source '%s' in %d hardware configurations for PTP profile %s",
		conditionType, sourceName, len(relevantConfigs), *nodeProfile.Name)
