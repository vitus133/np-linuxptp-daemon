...
func TestUpdateLeadingClockData_PTP4lProcessName(t *testing.T) {
	event := EventChannel{
		...
		Values: map[ValueType]interface{}{
			TIME_PROPERTIES_DATA_SET: &protocol.TimePropertiesDS{},
			CONTROLLED_PORTS_CONFIG:  "config",
			PARENT_DATA_SET:          &protocol.ParentDataSet{},
			CURRENT_DATA_SET:         &protocol.CurrentDS{StepsRemoved: 10},
		},
	}

	expectedLeadingClockData := LeadingClockParams{
		upstreamTimeProperties:        event.Values[TIME_PROPERTIES_DATA_SET].(*protocol.TimePropertiesDS),
		controlledPortsConfig:         "config",
		upstreamParentDataSet:         event.Values[PARENT_DATA_SET].(*protocol.ParentDataSet),
...
func TestUpdateLeadingClockData_DPLL(t *testing.T) {
	event := EventChannel{
		...
		Values: map[ValueType]interface{}{
			LEADING_SOURCE:              true,
			IN_SYNC_CONDITION_THRESHOLD: uint64(100),
			IN_SYNC_CONDITION_TIMES:     uint64(200),
			TO_FREERUN_THRESHOLD:        uint64(300),
			MAX_IN_SPEC_OFFSET:          uint64(400),
		},
	}
